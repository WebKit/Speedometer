...
export const MiniOverview = () => {
    ...
    const getCatBounds = (type, defaultMin, defaultMax) => {
    ...
    const getMovingAverageLog = (targetYear, points, windowYears = 8) => {
    ...
    const getStartYear = (points, fallback) => {
        if (points.length === 0)
            return fallback;
    ...
    const getTrendPoints = (start, end, points, type) => {
        const map = new Map();
    ...
    for (let y = 1900; y <= 2020; y += 10)
        decades.push(y);
    function updateMarkersCache(data) {
        markersCache = [];
        if (!data)
            return;
    ...
    return {
        ...
        view(vnode) {
            ...
            const hoverDensities = hoveredTags
                .map((category) => {
                const categoryCards = sourceData.filter((c) => c && c.tags && c.tags.includes(category));
                const density = calculateDensityCurve(categoryCards, { startYear, endYear, windowYears: 6, maxHeight: 85 });
                const tagInfo = TAGS[category] || TAGS.default;
            ...
            const pointsAtYear = [
                hoverCPU !== null ? { y: getYForType(hoverCPU, "cpu"), type: "cpu", color: "#00f0ff" } : null,
                hoverGPU !== null ? { y: getYForType(hoverGPU, "gpu"), type: "gpu", color: "#39ff14" } : null,
                hoverTPU !== null ? { y: getYForType(hoverTPU, "tpu"), type: "tpu", color: "#ff00ff" } : null,
            ].filter(Boolean);
            ...
            return m("#mini-overview-container", {
                ...
                m(".interactive-area", {
                    ...
                    m(".computation-graph-row", [
                        ...
                        }, processedFlops.map((p) => m("div.flops-data-dot", {
                        ...
                        m(".flops-hover-tooltip.glass-panel", {
                            style: {
                            ...
                            onpointerdown: (e) => {
                                ...
                                const handlePointerMove = (moveEvent) => {
                                    ...
                                    animationFrameId = requestAnimationFrame(() => {
                                        ...
                                        let l = 0, r = markersCache.length - 1;
                    ...
                    m(".data-marker-row", [
                        ...
                        dataHoverState
                            ...
                            && m(".data-hover-tooltip.glass-panel", {
                                style: {
