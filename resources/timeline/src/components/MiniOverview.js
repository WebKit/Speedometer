...
export const MiniOverview = () => {
    ...
    const generateTrendPath = (start, end, points) => {
        const pts = [];
    ...
    return {
        ...
        view(vnode) {
            ...
            const pointsAtYear = [
                hoverCPU !== null ? { y: getY(hoverCPU), type: "cpu", color: "#00f0ff" } : null,
                hoverGPU !== null ? { y: getY(hoverGPU), type: "gpu", color: "#39ff14" } : null,
                hoverTPU !== null ? { y: getY(hoverTPU), type: "tpu", color: "#ff00ff" } : null,
            ].filter(Boolean);
