...
const FolderWrapper = (props) => {
    ...
    for (let i = 0; i < numChildren && nodeCount.current < TARGET_SIZE; i++) {
        children.push(<TreeItem key={i} nodeCount={nodeCount} rando={rando} numChildren={numChildren} maxBreadth={maxBreadth} maxDepth={maxDepth} childProb={childProb} currentDepth={currentDepth + 1} />);
    }
...
const TreeItem = (props) => {
    ...
    const children = rando.coin(childProb) ? <FolderWrapper nodeCount={nodeCount} rando={rando} maxBreadth={maxBreadth} maxDepth={maxDepth} childProb={childProb} currentDepth={currentDepth + 1} /> : null;
    return (
...
export const TreeArea = (props) => {
    ...
    return (
        <div className="ui tree-area">
            <FolderWrapper nodeCount={nodeCount} rando={rando} maxBreadth={maxBreadth} maxDepth={maxDepth} childProb={childProb} currentDepth={0} />
        </div>
    );
};
