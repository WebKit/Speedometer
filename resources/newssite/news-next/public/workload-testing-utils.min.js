...
export class BenchmarkTestStep {
    ...
    async runAndRecord({ params, suitename, callback }) {
        ...
        const invokerClass = measurementMethod === "raf" ? RAFTestInvoker : TimerTestInvoker;
        const runSync = () => {
            if (warmupBeforeSync) {
                performance.mark("warmup-start");
                const startTime = performance.now();
                // Infinite loop for the specified ms.
                while (performance.now() - startTime < warmupBeforeSync)
                    continue;
        ...
        const invoker = new invokerClass(runSync, measureAsync, report, waitBeforeSync);
